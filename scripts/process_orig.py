"""
This script takes args to convert an 'orig' dataset
"""
...
def tokenize(text):
    return [tok.text.lower() for tok in tokenizer(text.strip())]
...
class VocabEntry(object):
    ...
    def from_corpus(fname):
        ...
        with open(fname) as fin:
            for line in fin:
                _ = [vocab.add(word) for word in tokenize(line)]
        ...
        with open(fname) as fin:
            for line in fin:
                if label:
                    ...
                    lb = split_line[0]
                    split_line = tokenize(split_line[1])
                else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="process the given input directory")
    
    parser.add_argument("data", type=str, help="i/o directory")
    parser.add_argument("train", type=str, help="train file name")
    parser.add_argument("val", type=str, help="val file name")
    parser.add_argument("test", type=str, help="test file name")
    parser.add_argument("l", type=str, help="l extension")
    parser.add_argument("r", type=str, help="r extension")
    parser.add_argument("s0", type=str, help="name of style 0")
    parser.add_argument("s1", type=str, help="name of style 1")    
    ...
    orig_dir = f"{save_dir}_orig"

    train0 = f"{orig_dir}/{args.train}.{args.l}"
    train1 = f"{orig_dir}/{args.train}.{args.r}"
    dev0 = f"{orig_dir}/{args.val}.{args.l}"
    dev1 = f"{orig_dir}/{args.val}.{args.r}"
    test0 = f"{orig_dir}/{args.test}.{args.l}"
    test1 = f"{orig_dir}/{args.test}.{args.r}"

    filelist = [train0, train1, dev0, dev1, test0, test1]
    with open(os.path.join(save_dir, "tmp.txt"), 'w') as file:
        input_lines = fileinput.input(filelist)
        file.writelines(input_lines)

#    subprocess.run("{} {} {} {} {} {} {} > {}".format(cmd, train0, train1, 
#        dev0, dev1, test0, test1, os.path.join(save_dir, "tmp.txt")), shell=True)
    ...
    with open(train0, "r") as fin:
        for line in fin:
            ftrain_t.write(line)
            ftrain_a.write(f"{args.s0}\n")

    with open(train1, "r") as fin:
        for line in fin:
            ftrain_t.write(line)
            ftrain_a.write(f"{args.s1}\n")
    ...
    ftrain_t = open(os.path.join(save_dir, "test_sub.txt"), "w")
    ftrain_a = open(os.path.join(save_dir, "test_sub.attr"), "w")
